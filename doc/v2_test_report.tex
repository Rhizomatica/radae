...
This section describes the End of Over (EOO) system performance.  When the transmission is about to cease, a special EOO frame is sent to instruct the receiver to immediately stop decoding.  This prevents decoding of spurious received signal and unwanted ``R2D2" tones emerging from the decoder at the end of a transmission.  The design goal is reliable operation in high SNR benign channels.  In poor channels (e.g. low SNR MPP), it is acceptable to have a lower probability of EOO detection, as clean end of over behaivour is a secondary requirement in the low SNR use case.  In all channels, should the EOO frame not be detected, the hangover counter in the receiver state machine will eventually stop decoding after a few seconds.

The script \emph{eoo\_detect\_prob.sh} generates $N=20$ by 9 \si{s} duration samples (extracted from \emph{all.wav}) each terminated in an EOO frame.  The number of sucessful EOO detections is counted to determine the probability of EOO detection for each condition.  The 9 \si{s} duration is chosen to ensure the alignment shifts between \emph{all.wav} and \emph{g\_mpp.f32} as the trials modulo rotate around each file.  Note the sharp knee in the AWGN performance at around 1 \si{dB}.
...
Figure \ref{fig:v2_snr_est} illustrates the SNR estimator performance.  SNR estimation for RADE V2 is challenging due to the lack of pilot symbols.  The algorithm used is based on the autocorrelation of cylic prefix and works well for AWGN channels, but accuracy suffers in multipath channels.  A straight line correction was applied to minimise overall the error for three common channels over the -5 to +5 \si{dB} range.  It is most accurate for slowly fading channels (MPG).

\begin{figure}[H]
\caption{Mean estimated SNR against actual SNR for several channels with noise measured in a 3 \si{kHz} bandwdth. Over the -5 to +5 dB interval the RMS error is 1.5 \si{dB} with worst case 2.8 \si{dB}.}
...
TODO: copy Mar blog post text and results.  Maybe get a few more results from Lee, or a third party, so it get's tested in different ways.
...
25 bit/s streaming text for callsign & Incomplete & Waveform can send 1 bit/frame over channel, but streaming text support not implemented yet. \\
