...
This section describes initial OTA tests using the stored file test system implemented by the script \emph{ota\_test.sh}.  The purpose of these tests was (a) see if RADE V2 actually works over real world HF channels (b) compare V2 to RADE V1 to determine if there is any improvement at low and high SNRs.  The simulated results presented above predict a 3 dB increase in sensitivity at low SNRs.  At high SNRs the ML network “loss” is significantly lower for V2, but we are not sure what that means in terms of speech quality.
...
In Table \ref{tab:conclusion} the results from this test campaign are summarised and compared to requirements extracted from FreeDV-038.  Only performance and functional requirements have been extracted (e.g. we don't consider implementation issues like a C port or packaging).
...
\hline
Speech quality v SNR $\ge$ RADE V1 on AWGN \& MPP. & Pass & $\approx 3$ \si{dB} gain in simulations, supported by OTA tests. \\
...
\item Write a ``type approval" procedure to allow 3rd parties to verify their integration of RADE V2.
\end{enumerate}
